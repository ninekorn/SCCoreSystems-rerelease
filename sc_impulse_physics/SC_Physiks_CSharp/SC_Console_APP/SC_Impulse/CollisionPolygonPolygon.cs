...
namespace SCCoreSystems
{
    ...
    public class CollisionPolygonPolygon : CollisionCallback// implements CollisionCallback
    {
        public static CollisionPolygonPolygon instance = new CollisionPolygonPolygon();

        public void handleCollision(Manifold m, Body a, Body b) //out SC_RigidInfo object1, out SC_RigidInfo object2, int checkOne, SC_RigidInfo singleObject, out Vector2? intersectPoint, float theTime
        {
        ...
        public int clip(Vec2 n, float c, Vec2[] face)
        {
            ...
            if (d1 <= 0.0f) outVecs[sp++].set(face[0]);
            if (d2 <= 0.0f) outVecs[sp++].set(face[1]);
            ...
            if (d1 * d2 < 0.0f) // less than to ignore -0.0f
            {
                ...
                outVecs[sp++].set(face[1]).subi(face[0]).muli(alpha).addi(face[0]);
            }
