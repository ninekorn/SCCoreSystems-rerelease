...
namespace SCCoreSystems.sc_console
{
    public class SC_Update : SC_console_directx
    {
        ...
        private unsafe SC_message_object_jitter[][] _FrameVRTWO(jitter_sc[] jitter_sc, SC_message_object_jitter[][] _sc_jitter_tasks)
        {
            ...
            //WRITE WORLDMATRIXES OF EACH INSTANCES TO THE DEDICATED SINGLE BUFFER (CONTAINING ONLY THE TOTAL INSTANCES OF THE SAME OBJECT) THAT IS SENT TO THE SHADERS.
            //WRITE WORLDMATRIXES OF EACH INSTANCES TO THE DEDICATED SINGLE BUFFER (CONTAINING ONLY THE TOTAL INSTANCES OF THE SAME OBJECT) THAT IS SENT TO THE SHADERS.
            //WRITE WORLDMATRIXES OF EACH INSTANCES TO THE DEDICATED SINGLE BUFFER (CONTAINING ONLY THE TOTAL INSTANCES OF THE SAME OBJECT) THAT IS SENT TO THE SHADERS.
            try
            {
            ...
            //OCULUS RIFT CV1 EYE TEXTURE COMMIT
            //OCULUS RIFT CV1 EYE TEXTURE COMMIT
            //OCULUS RIFT CV1 EYE TEXTURE COMMIT
            try
            {
                ...
                for (int eyeIndex = 0; eyeIndex < 2; eyeIndex++)
                {
                    ...
                    if (_can_work_physics == 1)
                    {
                        _sc_jitter_tasks = _graphics_sec.workOnSomething(_sc_jitter_tasks, viewMatrix, _projectionMatrix, OFFSETPOS, originRot, rotatingMatrix, hmdmatrixRot, hmd_matrix, rotatingMatrixForPelvis, _rightTouchMatrix, _leftTouchMatrix, handPoseRight, handPoseLeft , _desktopFrame); // ?? i cant call the abstract function.                                                                                                                                                                                                                                               //_sc_jitter_tasks = MainWindow._graphics_sec.workOnSomething(_sc_jitter_tasks, viewMatrix, _projectionMatrix, OFFSETPOS, originRot, rotatingMatrix, rotatingMatrixForPelvis, _rightTouchMatrix, _leftTouchMatrix, handPoseRight, handPoseLeft); // ?? i cant call the abstract function.
                    }
