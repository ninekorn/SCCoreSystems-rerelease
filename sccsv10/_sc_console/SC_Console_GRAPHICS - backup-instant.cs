...
namespace _sc_core_systems
{
    public class SC_Console_GRAPHICS
    {
        ...
        int _inst_spectrum_z = 420; // 36 // 210 //75 //5625
        ...
        byte[] _sound_byte_array = new byte[176400];
        byte[] _sound_byte_array_instant = new byte[176400]; //44100 //176400
        ...
        private sc_message_object.sc_message_object[] Render(sc_message_object.sc_message_object[] _main_received_object)
        {
            ...
            screenNormal = _getDirection(Vector3.Right, _quat_screen00);
            ...
            if (has_spoken_sec == 1)
            {
                ...
                for (int x = -widthL; x <= widthR; x++)
                {
                    ...
                    {
                        for (int z = -depthL; z <= depthR; z++, count_spec++)
                        {
                            //Program.MessageBox((IntPtr)0, "" + "0", "sccoresystems", 0);
                            ...
                            try
                            {
                                ...
                                if (_index < _world_spectrum_list[0]._arrayOfInstances.Length) //_inst_spectrum_x * _inst_spectrum_y * _inst_spectrum_z)
                                {
                                    if (_sound_byte_array != null)
                                    {
                                        if (_sound_byte_array.Length > 0)
                                        {
                                            if (count_spec < _sound_byte_array.Length)
                                            {
                                                ...
                                                spectrum_mat.M42 = _left_touch_pos.Y + _world_spectrum_list[0]._arrayOfInstances[_index]._POSITION.M42 + spectrum_noise_value + (_sound_byte_array[count_spec] * 0.00030f); //0.0015f
            ...
            if (has_spoken_tier == 1)
            {
                instant_short_path = "wave_instant_audio_" + _records_instant_counter;
                var filename = @"C:\Users\ninekorn\Desktop\#RECSINSTANTSOUND\" + "wave_instant_audio_" + _records_instant_counter + ".wav";
